...
import { RepoContainer, Status } from "./styles";
...
export const Repositories = ({ dataProfiles }: ReposProps) => {
  const pathParams = useParams();

  const [repos, setRepos] = useState([]);

  const getRepos = async () => {
    const response = await axios.get(`${BASE_URL}/${pathParams.repos}/repos`);
  ...
  return (
    <RepoContainer>
      <header>
        ...
        <h2>
          Repositories <span>{dataProfiles.public_repos}</span>{" "}
        </h2>
      </header>
      <nav>
        {repos.map((repo: RepoMapProps) => (
          <main key={repo.name}>
            <h3>
            ...
            <div>
              ...
              {repo.forks_count > 0 && (
                <>
                  <p>
                    <GitFork size={16} weight="thin" /> {repo.forks_count}
                  </p>
                </>
              )}
              ...
              {repo.license && (
                <>
                  <p>
                    {" "}
                    <Scales size={20} weight="thin" /> {repo.license}
                  </p>
                </>
              )}

              <p>update: on {moment(repo.updated_at).format("DD MMMM")}</p>
            </div>
            <hr />
          </main>
        ))}
